...
// Glass intensity presets - uses semantic tokens
const glassIntensity = {
  light: "bg-card/50 backdrop-blur-sm",
  medium: "bg-card/70 backdrop-blur-md",
...
export default function GlassCard({
  ...
  const gradientBorderStyles = gradient ? `
    ...
    after:rounded-${rounded} after:bg-card
    after:content-['']
  ` : "";
...
export function FeatureCard({ icon: Icon, title, description, gradient = false, className = "" }) {
  return (
    <GlassCard gradient={gradient} className={`p-6 group ${className}`}>
      {Icon && (
        <div className="w-14 h-14 rounded-xl bg-gradient-to-r from-gradient-from to-gradient-to flex items-center justify-center mb-4 group-hover:scale-110 transition-transform duration-300">
